...
@interface MXVBaseViewController ()

@end

@implementation MXVBaseViewController
...
- (void)viewDidLoad
{
    ...
    //系统返回按钮：隐藏文字、自定义文字、返回按钮的   
    self.navigationItem.backBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"" style:UIBarButtonItemStylePlain target:nil action:nil];  
    //返回按钮的箭头   
    [self.navigationController.navigationBar setTintColor:[UIColor colorWithHex:@"#88888"]];
...
- (void)zm_setNavTitle:(NSString *)str backBtnHidden:(BOOL)hidden originalBack:(BOOL)originalBack {
    ...
    MXVNavigationBarView *nav = [[MXVNavigationBarView alloc] init];
    nav.backButton.hidden = hidden;
    nav.title = str;
    [self.view addSubview:nav];
    self.zmNavBarView= nav;
    __weak __typeof(&*self)weakSf = self;
    if (originalBack) {
        nav.zmBackBlock = ^{
