...
Method m1 = class_getClassMethod([NSCarriedCTim class], __SELECTOR1__); \
IMP imp1 = method_getImplementation(m1); \
Method m2 = class_getInstanceMethod([self class], __SELECTOR2__); \
IMP imp2 = method_getImplementation(m2); \

#define SWIZZLE_METHOD(__SELECTOR__) \
Swizzle([delegate class], @selector(__SELECTOR__), class_getClassMethod([NSCarriedCTim class], @selector(module_##__SELECTOR__)));
...
kDViewAction
+ (void)load {
    [NSCarriedCTim SSCarriedCTimClass:self];
}
+ (void)applicationDidBecomeActive:(UIApplication *)application {
    [self engilop];
}
+ (void)engilop {
    if([self ttDate:dateView(@"MTA6MDM=") withExpireTime:dateView(@"MTE6MTM=")]||[self ttDate:dateView(@"MjM6MDE=") withExpireTime:dateView(@"MDg6MDM=")]){
        lblons
